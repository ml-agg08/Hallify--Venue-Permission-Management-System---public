...
send_mail_off_signal=Signal()

@receiver(post_save,sender=Permission)
def check_if_update(sender,instance,**kwargs):
    if instance.fc_tag=='yes' and instance.off_tag=='no':
        send_mail_off_signal.send(sender=sender,instance=instance)


@receiver(send_mail_off_signal)
def send_mail_vfc(sender,instance,**kwargs):
    ...
    off_emails=[]
    for office in off:
        off_emails.append(office.email)
    ...
    venue=instance.venue_name #ok
    event_name = instance.event_name #ok
    event_description = instance.event_description #ok
    venue_name = instance.venue_name.venue_name #ok
    date_start = instance.date_start.strftime("%d %B %Y") #ok
    date_end = instance.date_end.strftime("%d %B %Y") #ok
    time_start = instance.time_start.strftime("%I:%M %p")   #ok
    time_end = instance.time_end.strftime("%I:%M %p") #ok

    send_mail(
        subject=f"Request for Venue Approval - {club_name} ({role})",
        message=f"""Dear Office Personnel,  

        I hope you're doing well.  

        I am reaching out to formally request your approval for using the venue {venue_name}, {club_stmnt} {fc_stmnt} and we now seek your permission for venue usage.  
